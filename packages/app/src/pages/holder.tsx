...
const HolderPage: NextPage = () => {
  ...
  const calculateProofAndVerify = async () => {
    try {
      ...
      setProofMessage(`✅ Proof generated and validated successfully. Signature code: ${signature}`);
    } catch (e) {
      console.error(e);
    } finally {
      setIsLoading(false);
  ...
  return (
    <Layout>
      <Unit header={"Currently held zkSBTs:"}>
        <Stack spacing="4">
        {connected ? (
            <Stack>
              <Stack spacing="1">
              ...
              <CredentialCard
                ...
                credentialType="Franklin Templeton Blockchain Contest Attendee"
                operator={"="}
                value={"True"}
              />
            </Stack>
            ...
            <Text fontSize="xs" fontWeight={"bold"}>
              None. Please connect your wallet.
            </Text>
          )}
          {credentials.length > 0 && (
            <Stack>
              <Stack spacing="1">
                ...
                <Text fontSize="x-small" color="accent">
                  * Only one credential is allowed per wallet for demo purposes
                </Text>
              </Stack>
              <Stack>
